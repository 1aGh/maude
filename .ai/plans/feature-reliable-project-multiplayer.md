...
| T30 | Every crash seam its Validate line names is now covered — before the import (`mode-switch-resume`), inside it (`baseline-import-resume`), re-entry, and fencing. Two named seams turned out to be covered or absent: a socket held open across the switch IS the stale-loopback shape and its injected bytes are already refused, and there is no barrier response to lose (the switch broadcasts, waits a fixed grace, fences per message — pinned by `mode-switch-barrier`). What remains is the rollback boundary itself: the runbook documents restoring before and after a new accepted action, and no test or drill exercises it. |
