...
#### `integrations.tracker.artifacts`
...
##### `integrations.tracker.artifacts.store`

**Type:** enum: `"local"` │ `"orbit"` │ `"both"` · **Default:** `"local"`

`local` — files in `.ai/` are the record (close-time push still applies). `orbit` — orbit is the record: each artifact is pushed the moment it is written and no copy is kept in the repo. `both` — write locally AND push at write time; the migration setting, and the safe way to try `orbit` out.
...
##### `integrations.tracker.artifacts.local`

**Type:** enum: `"keep"` │ `"scratch"` · **Default:** `"keep"`

Only meaningful with `store: orbit`. `scratch` — the command writes its artifact under `spoolDir` (gitignored), pushes it, and deletes it on a confirmed push; a FAILED push leaves the file there and the next flow command retries it. `keep` — write to the usual `.ai/` path anyway, so a repo mid-migration keeps its history. A file is never deleted before orbit confirms.
...
##### `integrations.tracker.artifacts.spoolDir`

**Type:** `string` · **Default:** `".ai/tmp/orbit-spool"`

Directory for scratch artifacts awaiting a confirmed push, relative to repo root. Must be gitignored — `/flow:init` adds it to `.ai/.gitignore`. Nothing else ever writes here and no command reads it except the spool retry.

##### `integrations.tracker.artifacts.kinds`

**Type:** `enum("plan", "rca", "execution-report", "retro", "review")[]` · **Unique items**

Which artifact kinds this repo routes to orbit. Absent → all five. A kind left out behaves as `store: local` for that kind alone; orbit accepts no other kinds.
