...
| H09 | [studio/git/service.ts](../../apps/studio/git/service.ts): `gitDiscard`, `gitCheckout`, `gitFoldDraft`, `gitPull`, `gitResolve` (including conflict-copy writes); HTTP `/_api/git/discard`, `/checkout`, `/fold`, `/pull`, `/resolve` | Managed checkout cannot become a second authority; import explicit Git action/candidate or disallow on managed accepted view. T14/T15/T27/T30; A,S,M,F,R |
